...
public class SquaredHingeLossTermTest {
    ...
    public void testReadWrite() throws IOException {
        ...
        SquaredHingeLossTerm term = new SquaredHingeLossTerm(new Hyperplane<LocalVariable>(variables, coeffs, constant, z.length), wInd);
        ...
        SquaredHingeLossTerm term1 = new SquaredHingeLossTerm(new Hyperplane<LocalVariable>(variables1, coeffs1, constant1, z1.length), wInd1);

        Assert.assertEquals((Float.SIZE*3 + Integer.SIZE * 4) / 8, term.fixedByteSize()); // 2 coeff + const + ruleIndex + 2 globalId + size
    ...
    public void testReadWrite2() throws IOException {
        ...
        Assert.assertEquals((Float.SIZE*2 + Integer.SIZE * 3) / 8, term1.fixedByteSize()); // 1 coeff + const + ruleIndex + 1 globalId + size
    ...
    private void testProblem(float[] z, float[] y, float[] coeffs, float constant,
        ...
        SquaredHingeLossTerm term = new SquaredHingeLossTerm(new Hyperplane<LocalVariable>(variables, coeffs, constant, z.length), 0);
