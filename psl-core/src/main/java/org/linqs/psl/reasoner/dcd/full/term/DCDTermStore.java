...
public class DCDTermStore implements TermStore<DCDObjectiveTerm> {
	...
	private Map<AtomFunctionVariable, ArrayList<DCDObjectiveTerm> > variableToTerm;
	...
	public DCDTermStore(TermStore<DCDObjectiveTerm> store) {
		...
		numLocalVariables = 0;
		variableToTerm = new HashMap<AtomFunctionVariable, ArrayList<DCDObjectiveTerm>>();
	}

	/**
	 * Create a local variable and ensure that a global copy is registered.
	 */
	public synchronized void registerTerm(DCDObjectiveTerm term) {
		for (AtomFunctionVariable atomVariable : term.getVariables()) {
			...
			if (!variableIndexes.containsKey(atomVariable)) {
				...
				variableToTerm.put(atomVariable, new ArrayList<DCDObjectiveTerm>());
			}
			List<DCDObjectiveTerm> terms = variableToTerm.get(atomVariable);
			terms.add(term);
		}

	}

	public List<DCDObjectiveTerm> getTermsUsingVar(AtomFunctionVariable var){
