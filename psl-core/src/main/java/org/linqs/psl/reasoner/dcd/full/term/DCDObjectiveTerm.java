...
public abstract class DCDObjectiveTerm implements WeightedTerm  {
	...
	public float computeGradient(float dots, float aux) {
		return dots + constant - aux;
	}
	 ...
	 */
	public abstract void minimize(float alphaDotQi, float aux);
	 ...
	 */
	protected void minimize_(float grad, float lim) {
		float pg = grad;
		if (lagrangeVar == 0){
			pg = Math.min(grad, 0);
		}
