...
public class DRLStructureLearner extends AbstractStructureLearningApplication{
    ...
    public static DQNFactoryStdDense.Configuration PSL_NET =
            DQNFactoryStdDense.Configuration.builder()
                    .l2(0.01).updater(new Adam(1e-2)).numLayer(2).numHiddenNodes(16).build();
    ...
    private void populateNextRulesOfModel() throws IOException {
        ...
        psl_ql.batchSize(12).maxEpochStep(20).maxStep(50000).expRepMaxSize(1).batchSize(32).targetDqnUpdateFreq(32).updateStart(0).rewardFactor(1).gamma(0.98).errorClamp(10000).minEpsilon(0.1f).epsilonNbStep(3000).doubleDQN(true);
        QLearning.QLConfiguration qlConfiguration = psl_ql.build();

        QLearningDiscreteDense<State> dql = new QLearningDiscreteDense<State>(mdp, PSL_NET, qlConfiguration, manager);
